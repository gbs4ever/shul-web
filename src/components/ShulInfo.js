import React from 'react'

class ShulInfo extends React.Component {
  render() {
    return (
      <div className="actions">
        Please take this opportunity to help make our kehilla and shul complete.
        Membership for our bais medrash is $750 a year.<br></br> Checks can be
        made out to Cong Torah Utfilla.<br></br> To pay by credit card or to set
        up recurring monthly payments.<br></br>
        <a href="https://secure.cardknox.com/torahutfilla" className="button">
          click here
        </a>{' '}
        .
      </div>
    )
  }
}

export default ShulInfo
